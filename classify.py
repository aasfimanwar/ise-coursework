...
def make_proposed():
    ...
    return Pipeline([
        ...
        ("clf", LinearSVC(
            class_weight="balanced",   # handles the ~16% positive imbalance
            C=1.0,
            max_iter=2000,
...
def single_run(texts, labels, seed):
    """One train/test split, return metrics for both approaches."""
    rng = np.random.RandomState(seed)
    indices = np.arange(len(texts))
    rng.shuffle(indices)

    # ensure positive samples in both splits
...
def main():
    ...
    #Save raw arrays for replication
    raw = {}
    for project in PROJECTS:
        if all_results.get(project) is None:
            continue
